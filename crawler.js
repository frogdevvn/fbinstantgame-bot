...
red.on('error', (err) => {
  console.log(`${new Date().toString()} || Redis error: ${err}`);
});
...
function CanSend(tsm, days) {
  if (tsm >= SendRegularity.length) return false;
  if (days > SendRegularity[tsm]) return true;
  return false;
}
...
etInterval(ProcessPlayers, 30000);
